...
echo -e "threshold\talpha\tMean/MultiLabel-F0.5\tMean/MultiLabel-F2\tF0.5\tF2\tAccFstError\tAccAnyError" | tee -a results_gpt2-large.tmp
for alpha in $(seq 0.05 0.10 7.0); do
  alpha=$(echo $alpha | tr "," ".")
  ...
  echo -e "${result}" | tee -a results_gpt2-large.tmp
done
